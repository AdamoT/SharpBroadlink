...
namespace SharpBroadlink.Devices
{
	public class Sp2 : Device
	{
        ...
        public async Task<(bool Power, bool NightLight)> CheckStatus()
		{
			...
			if (err == 0)
			{
				var payload = Decrypt(response.Skip(0x38).Take(int.MaxValue).ToArray());
		...
		{
			return (await CheckStatus()).Power;
		}
		...
		{
			return (await CheckStatus()).NightLight;
		}
        ...
        public async Task<bool> SetPower(bool state)
		{
			...
			if ((await CheckStatus()).NightLight)
