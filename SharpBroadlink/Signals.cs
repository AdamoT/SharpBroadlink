...
namespace SharpBroadlink
{
    ...
    public static class Signals
	{
        ...
        public static byte[] Lirc2Broadlink(int[] raw)
		{
			...
			foreach (var elem in raw)
			{
				...
				else
				{
					...
					array.Add(0x00);
					// big endian (2-bytes)
					var bytes = BitConverter.GetBytes(calcedPulse);
					array.Add(bytes[1]);
					array.Add(bytes[0]);
				}
			}
		...
		{
			var length = BitConverter.ToInt16(new[] {bytes[2], bytes[3]}, 0);
			...
			while (true)
			{
				...
				if (bytes[index] == 0x00)
				{
					...
					elem = BitConverter.ToUInt16(new[] {bytes[index + 2], bytes[index + 1]}, 0);
					index += 3;
				}
				else
				{
		...
		{
			var raw = Broadlink2Lirc(bytes);
			var result = Lirc2Pronto(raw, frequency);
			return result;
		}
	}
}